...
#     bbox = bbox.astype(np.int)
#     return img, bbox
# def translation(img, bbox, category):
#     factor_horizontal = random.uniform(-0.2, 0.2)
#     factor_vertical = random.uniform(-0.2, 0.2)
#     h, w, _ = img.shape
#     w_tran = int(w * factor_horizontal)
...
#     print((bbox[0, 0] + bbox[0, 2])/2, (bbox[0, 1]+bbox[0, 3])/2)
#     print((bbox[1, 0] + bbox[1, 2]) / 2, (bbox[1, 1] + bbox[1, 3]) / 2)
#     category = np.array([11, 14])
#     img, bbox, category = translation(img, bbox, category)
#     h, w, c = img.shape
#     img = cv.resize(img, (448, 448))
#     bbox[:, [0, 2]] = bbox[:, [0, 2]] * (448 / w)
#     bbox[:, [1, 3]] = bbox[:, [1, 3]] * (448 / h)
#     img1 = img.copy()
#     for idx in range(len(bbox)):
#         cv.rectangle(img1, (bbox[idx, 0], bbox[idx, 1]), (bbox[idx, 2], bbox[idx, 3]), (0, 255, 0), 1, 1)
#     cv.imshow("test", img1)
#     target = encoder(img, bbox, category) # 7x7x30
#     each_cell = 448 / 7
#     # the top-left coordinate of grid cells on original images
#     horizontal_lt = np.arange(7)
#     horizontal_lt = np.expand_dims(horizontal_lt, axis=0) # 1, 7
#     horizontal_lt = np.repeat(horizontal_lt, 7, axis=0)  # 7, 7
#     horizontal_lt = horizontal_lt * each_cell
#     vertical_lt = np.arange(7)  # 7, 1
#     vertical_lt = np.expand_dims(vertical_lt, axis=-1) # 7, 1
#     vertical_lt = np.repeat(vertical_lt, 7, axis=1)  # 7, 7
#     vertical_lt = vertical_lt * each_cell
#
#     target[:, :, 0] = target[:, :, 0] * each_cell + horizontal_lt
#     target[:, :, 1] = target[:, :, 1] * each_cell + vertical_lt
#     target[:, :, 2] = target[:, :, 2] * 448
#     target[:, :, 3] = target[:, :, 3] * 448
#
#     xmin = target[:, :, 0] - target[:, :, 2] / 2
#     ymin = target[:, :, 1] - target[:, :, 3] / 2
#     xmax = target[:, :, 0] + target[:, :, 2] / 2
#     ymax = target[:, :, 1] + target[:, :, 3] / 2
#     category = np.argmax(target[:, :, 10:], axis=-1)
#     hasObj = (target[:, :, 4]==1)
#     xmin = xmin[hasObj].reshape(-1)
#     ymin = ymin[hasObj].reshape(-1)
#     xmax = xmax[hasObj].reshape(-1)
#     ymax = ymax[hasObj].reshape(-1)
#     category = category[hasObj]
#     # print(xmin, ymin, xmax, ymax)
#     for x1, y1, x2, y2, c in zip(xmin, ymin, xmax, ymax, category):
#         print((x1+x2)/2, (y1+y2)/2)
#         cv.rectangle(img, (x1, y1), (x2, y2), (0, 255, 0), 1, 1)
#         print(c)
#     cv.imshow("test1", img)
#     cv.waitKey()
